...
- [[sage-payload-meta-override-pattern]] **`🔥 HUB`** — Паттерн правки тона Sage **БЕЗ** правки системного промпта (owner-mandate «промпт=бренд»). **6 живых META** (на 2026-06-12): time_meta_warning (vечер≥16:00, my_day), closed-budget directive, VARIATION GUARD (репит штампа), RULE 7 HARD GUARD (≥20г Б, food_log), COLD-START PHASE (0 логов + past breakfast, PR #385), BAN LIST (конкретные foods из последних реакций, sharper guard, PR #386). Порядок в payload = возрастание специфичности. Race-fix `just_emitted_food_log` (PR #383). Telemetry coverage via `metas_fired` jsonb (PR #385). Fail-safe = пустая строка при любом сбое источника.
