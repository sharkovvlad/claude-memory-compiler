...
| **🔥 Eval-каталог (все evals в одном hub)** | 🔥 `food-data-evals` — единый каталог: completed (FatSecret NO-GO N=20) + planned (CIQUAL/BEDCA) + future triggers + дизайн-принципы (sample-size floors, ground truth, multi-dim, что мерить кроме per-100g) |
| **Региональные food-composition БД (CIQUAL/BEDCA/IFCT)** | `regional-food-composition-databases` — deferred design: что это, зачем (региональные блюда), readiness-критерии (user-base + eval-результат + operational), implementation skeleton. Status: queued eval next session |
...
- [[sage-payload-meta-override-pattern]] **`🔥 HUB`** — Паттерн правки тона Sage через payload-META (owner-mandate «промпт=бренд», но эталоны voice cards добавлены в промпт PR #392+#394). **9 живых META** (на 2026-06-13): time_meta_warning (вечер≥16, my_day), closed-budget directive, VARIATION GUARD, RULE 7 HARD GUARD (food_log), COLD-START PHASE, BAN LIST, **late_night_close / late_night_soft** (h≥22, PR #392), **quiet_steady_no_push** (my_day hard guard, PR #396), **language_lock** (always-last, PR #396 hotfix Hindi→RU leak). Порядок в payload = возрастание специфичности; `language_lock` ВСЕГДА самый последний. Telemetry coverage via `metas_fired` jsonb. Voice cards (8 эталонов в системном промпте) + CULTURE GUARD (food substitution по 11 регионам, halal/vegetarian filter) — PR #394.
- [[sage-tone-dry-run-protocol]] **`🔥 HUB · MANDATORY pre-merge`** — Owner-mandated 2026-06-13. Любая правка системного промпта или payload-META обязана пройти synthetic dry-run ДО merge. 8 базовых сценариев (RU/ES/AR/HI/DE/FR/ID × food_log/my_day × разные day_status × разные часы) через `tools/sage_dry_run.py` → реальные LLM-вызовы → markdown digest в PR description. Found language_leak + quiet_steady_push bugs которые unit-тесты НЕ ловили (PR #396). Cost ~$0.05 + 5 мин; cost регресса — sale-blocker или РПП-trigger.
- [[sage-silent-presence-mode]] — Owner mandate (PR-3b, 2026-06-13): «если совета нет, может лучше меньше слов, просто поддержать». Новая META на food_log surface — параллель `quiet_steady_no_push` на my_day. Триггер: master flag enabled + meal balanced (fat ≤ 45% kcal) + day kcal ∈ [40, 95]% + wellbeing не красное (sleep≠short AND stress≠high). Suppressed когда budget_closed или late_night активен. 4 порога в `app_constants` (hot-reload). Card S в системном промпте как reference voice. Mig 507. Master flag по умолчанию **off** — safe rollout (рацио: изоляция переменной в watch). Содержит durable «voice card ≠ META» distinction. PR [#401](https://github.com/sharkovvlad/noms-bot/pull/401).
- [[handover/2026-06-13_sage-payload-contextualization-cycle]] **`📬 open handover`** — Цикл PR-3b→3c→3d (нутрициологическая индивидуализация payload Sage). Phase 1 ([#401](https://github.com/sharkovvlad/noms-bot/pull/401), mig 507) + Phase 2 ([#403](https://github.com/sharkovvlad/noms-bot/pull/403), mig 509 — RPC `get_weekly_pattern` 7-дневный тренд, Card Q, WEEKLY PATTERN AWARENESS) SHIPPED; Phase 3 (PR-3d: goal-vs-actual divergence hint, деликатный — anti-shame) — pending. Durable: новый payload context-сигнал требует directive-инструкции в промпте, иначе под-весится (dry-run поймал carb-push кето-юзеру). first-step: watch 24-48ч → потом silent_presence flip изолированно.
...
- [[subagent-live-apply-review-rule]] — Subagent LIVE-apply: orchestrator МUST review SQL before authorize. TaskStop ≠ rollback (P0 lesson 2026-05-26). **+ Parallel vs sequential (2026-06-12):** worktree-isolation parallel ONLY для truly-independent изменений в разных файлах; для co-located (один `services/sage.py` + 2 META) — sequential без isolation, иначе cherry-pick → conflict markers.
...
- [[fx-policy-revshare]] — FX policy для RevShare/комиссий: snapshot курса на дату операции (Big-Tech consensus), EUR валюта учёта (юрлицо Испания), Stars net base / card+USDT full base, ECB daily reference rate, `fx_rates_daily` audit, daily cron `FxRateUpdateCron`. mig 493 (2026-06-08).
