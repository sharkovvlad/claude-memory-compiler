...
## 6c. Onboarding UX simplification — APPROVED 2026-05-18 (late session)

Owner decision: **убрать кнопку «🤐 Не отвечу»** из onboarding maternal step и Profile picker. Финальная структура — **3 buttons**:

```
[ ✅ Нет, обычный режим ]    ← default low-friction
[ 🤰 Беременна ]
[ 🍼 Кормлю грудью ]
```

**Rationale:**
- `[ Нет, обычный режим ]` — уже low-friction explicit answer (90%+ non-pregnant women кликнут одним тапом)
- NULL state создаёт complexity в формулах (protective mode mig 254) — для новых юзеров не нужно
- 3 buttons clean'er 4 (cognitive load principle)

**Effect on calc logic:** mig 254 protective mode для F/15-50+NULL+lose остаётся как **defensive net** для legacy users (зарегистрированы до onboarding maternal step) + edge cases (admin imports, /support manual SQL). Не triggers для пост-onboarding users (они FALSE explicit).

**Effect on Profile picker (mig 257):** mig 260 удалила `cmd_set_pregnancy_unspecified` + `cmd_set_lactation_unspecified` buttons из `personal_metrics_set_pregnancy` + `_set_lactation` screens (consistency с onboarding). RPCs `set_user_pregnancy_flag` etc. сохраняют 'unspecified' handling для backwards compatibility + /support edge cases.
