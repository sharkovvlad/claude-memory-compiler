...
**Накопительный стенд (owner ask):** `run_eval.py` теперь печатает прод-снимок (real-логи + correction-rate за окно) — еженедельный прогон = датированная точка тренда. Live 2026-06-05: 378 логов / 167 блюд / correction-rate ~1.2% за 14д (и те 2 правки = owner-тесты → органически ≈0% → юзеры не воюют с распознаванием, что подтверждает no-go).
