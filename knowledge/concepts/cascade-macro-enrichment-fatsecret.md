...
> **Статус 2026-06-05:** Дешёвые рычаги LIVE (temperature/локация/память коррекций+refinement/portion_note). **🔴 Eval ЗАВЕРШЁН (PR #337) → FatSecret name-enrichment NO-GO:** GPT-4o уже даёт медиану 0% откл. на 100г (одиночные) / 1% (брендовые); FatSecret не превосходит (ничья/иногда хуже, промах 4/6 составных и обоих брендовых, fuzzy-шум egg→«Egg Foo Yung»). Главная ошибка = порция/состояние (закрыто portion_note+память), не плотность. Клиент FatSecret оставлен для barcode-2го-источника.
...
## 13. 🔴 EVAL РЕЗУЛЬТАТ (PR #337, 2026-06-05) — name-enrichment NO-GO

Стенд `tools/macro_eval/` (20 эталонных позиций, запуск с VPS) сравнил per-100g точность LLM vs FatSecret. **Медиана откл. ккал от эталона:**
- ВСЕ: LLM 0% / FS 0% (FS покрыл 14/20). Одиночные: LLM 0% / FS 0% (12/12). Составные: LLM 10% / FS 4% но покрыл лишь 2/6. **Брендовые: LLM 1% / FS промахнулся ОБА (0/2).** Кириллица: LLM 8% / FS 9% (через name_en, 2/4).
- Сюрпризы: курица LLM 0% / FS **18% хуже**; egg → FS матчит «Chicken Egg Foo Yung» (блюдо!) — fuzzy-шум.

**Вывод:** GPT-4o уже знает макросы на 100г отлично; FatSecret не превосходит + дыры покрытия + риск ложного матча + латентность. **name-enrichment НЕ строим.** Главный рычаг точности = порция/состояние (portion_note + память коррекций, оба LIVE), не плотность. Для брендового — штрихкод (точный), не name-search. Клиент FatSecret (PR #335) оставлен — может пригодиться barcode-2му-источнику. Caveat: эталоны ±10-15%, но направление однозначно. Результаты — `tools/macro_eval/results_2026-06-05.md`. 

**🔴 FatSecret barcode-2й-источник тоже NO-GO на free-tier:** `food.find_id_for_barcode` требует scope `barcode` (Premier/платный) — у нас только `basic` → `code 14: Missing scope 'barcode'` (verified VPS 2026-06-05). То есть ОБА применения FatSecret на бесплатном тарифе закрыты (name=нет выигрыша, barcode=платный). Для штрихкода остаётся OpenFoodFacts (бесплатный, трек barcode-агента). FatSecret-клиент оставлен на случай Premier.

**Накопительный стенд (owner ask):** `run_eval.py` теперь печатает прод-снимок (real-логи + correction-rate за окно) — еженедельный прогон = датированная точка тренда. Live 2026-06-05: 378 логов / 167 блюд / correction-rate ~1.2% за 14д (те 2 правки = owner-тесты → органически ≈0%). **⚠️ ВАЖНО (owner 2026-06-05): correction-rate ≈0 НЕ доказывает точность распознавания** — это silent-user bias (юзеры ленивы, не любят лишние нажатия; молчание ≠ корректность). Это метрика ГОТОВНОСТИ-ПРАВИТЬ, не точности. **FatSecret no-go держится на ОФФЛАЙН golden-set (vs reference-значения), НЕ на correction-rate.** И golden-set тестировал только per-100g ПЛОТНОСТЬ; ИДЕНТИФИКАЦИЯ блюда и ПОРЦИЯ на реальных фото/тексте — НЕ измерены → реальная точность распознавания пока неизвестна. Честно измерить = labeled real-input eval (полный pipeline: идентификация+порция+макросы).

---

## 13.1 🟡 STATISTICAL CAVEAT — выборка мала, вывод предварительный (owner 2026-06-08)

**Размер выборки:** golden-set = **20 позиций** (12 одиночных, 6 составных, 2 брендовых). Это **демо-уровень**, не статистически значимый sample.

**Что это значит:**
- ✅ Direction вывода правдоподобен: GPT-4o явно не катастрофически хуже FS, FS не показал систематического преимущества → нет очевидного value в name-enrichment СЕЙЧАС.
- ❌ Это НЕ финальное «GPT-4o лучше всегда». На N=20 confidence interval ширины «GPT 0%» легко может быть [0%, 15%] — мы просто не видим разницы.
- ❌ Особенно слабо покрытие на: составные блюда (6 позиций), брендовые (2 позиции), кириллица (4 позиции), региональные кухни (≈0).

**Когда пересматривать (re-eval triggers):**

| Условие | Действие |
|---|---|
| Активная база ≥100 DAU (любой регион) | Прогнать eval на расширенном golden-set (50+) |
| Новый major-релиз GPT (4o → 5, или аналог) | Прогнать существующий стенд + добавить новые сложные кейсы |
| Появление 20+ активных юзеров в одной стране (ES/MX/BR/IN/RU/IR/AR) | Запустить **региональный** eval — см. [[regional-food-composition-databases]] |
| Жалоба от юзера на устойчивую ошибку определённого класса блюд | Добавить этот класс в golden-set + прогон |
| Прошло >180 дней с последнего eval | Просто перепрогнать (training-data drift) |

**Статистический floor для серьёзного вывода:** N=100+ позиций per dimension (per region / per category) для CI ±5%. Сейчас мы на ~N=20 total → выводы качественные, не количественные.

**Sister-eval планируется:** CIQUAL/BEDCA/IFCT vs GPT-4o для региональных блюд (французская / испанская / индийская кухня). См. [[food-data-evals]] и [[regional-food-composition-databases]].

**Каталог всех evals:** [[food-data-evals]] — единый hub для discoverability (текущие + planned + результаты + readiness-критерии).
