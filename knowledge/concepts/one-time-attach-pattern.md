...
- **#11 — частично закрыт (mig 392, 2026-05-31):** `personal_metrics` получил `meta.reply_kb_entry=true` → после правки веса/возраста/роста в профиле (text_input → remove_keyboard) reply-kb теперь возвращается с тостом «✅ Сохранено!». До этого реальный юзер терял меню до /start (нажать «Мой день» нельзя — она на пропавшей kb). **Паттерн для будущего:** ЛЮБОЙ inline-экран, на который возвращаются после text_input-правки, должен иметь `meta.reply_kb_entry=true`, иначе reply-kb теряется. Остаётся открытым: generic re-attach для прочих root paths (cmd_get_stats/cmd_progress) если найдутся другие text_input-возвраты.
