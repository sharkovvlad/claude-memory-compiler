...
sources:
  ...
  - "migrations/493_ambassador_eur_net_commission.sql"
created: 2026-06-08
updated: 2026-06-08
---
...
### Snapshot колонки на `referral_rewards` (mig 493)
...
Партнёр на экране Статистика видит формулу под каждой строкой recent rewards (mig 493 рендерит inline в `get_ambassador_detail`):
```
• Евгения  +€1.63  (22.05)
  580⭐ × $0.013 × 25% × 0.862 = €1.63
```

Шаблоны по `base_currency` (генерируются напрямую в SQL):
- **XTR (Stars):** `{base_amount}⭐ × ${stars_payout_rate_used} × {commission_rate}% × {fx_used} = €{reward_value}`
- **USD/USDT:** `${base_amount} × {commission_rate}% × {fx_used} = €{reward_value}`
- **EUR:** `€{base_amount} × {commission_rate}% = €{reward_value}` (без FX, native EUR)

Formula line **скрывается** для строк до mig 493 (нет snapshot колонок) — IF `base_amount IS NULL` → fallback к голой `+€X.XX` без формулы. Так legacy строки (если появятся при ручном INSERT) не ломают экран.

**Архитектурное решение:** inline формула в RPC, не отдельный экран. Альтернатива (новый screen «Детали начисления» + кнопка на каждой строке) — over-engineering, формула уместна рядом с цифрой. Если в будущем нужны графики/история начислений — тогда отдельный экран.

## Per-method commission base (mig 493 owner decision)
...
## Backfill Евгении (mig 493 §J)

| Поле | До mig 493 | После mig 493 |
|---|---|---|
