...
**🔴 ВНИМАНИЕ (находка 2026-06-03, barcode-сессия): контракт-тест — ФАНТОМ на `main`.** Файл `tests/test_content_classification_contract.py` **НЕ существует в `main`**. CI-workflow `.github/workflows/pr-content-classification.yml` (PR #305, merged daf9157) ссылается на него и запускает `pytest tests/test_content_classification_contract.py …`, но сам тест остался на невмерженной ветке `claude/content-classification-fix` (коммит `14d3a17`, 26 кейсов) — PR #305 вмержил ТОЛЬКО yml. Следствие: (1) анти-дрифт-защита НЕ работает; (2) любой PR, трогающий `router.py`/`telegram_proxy.py`/`food_log.py`/`location.py`, запускает CI-job, который **падает на сборе** (file not found). **Быстрый фикс для всех:** cherry-pick `14d3a17` в main. Детали — [[barcode-logging-openfoodfacts]] §7(б).

**✅ Контракт-тест (дизайн, на ветке `14d3a17`):** `tests/test_content_classification_contract.py` пинит инвариант **«индикатор показывается ⟺ контент идёт в распознавание (food_media)»** для всех типов контента (photo/voice/image-doc → recognition+indicator; audio/video/sticker/poll/pdf/oversized-image-doc → unsupported, без indicator). Если кто-то изменит ОДИН классификатор и забудет другой — тест падает в CI/pre-push. **Заметку можно проигнорировать, красный тест — нет.** Запускать при любой правке классификации. (⚠️ восстановить в main — см. находку выше.)
