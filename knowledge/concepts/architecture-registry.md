...
| `JQsipPWxijse3F0b` | **04_Menu** (legacy) | 01_Dispatcher → "Go to 04_Menu" | 🟡 KEEP (усыхает) — остаток усох до **одного живого входа: reply 🍽 add_food**. «96 событий/2нед» были ДО late-May whitelist-фиксов; с 01.06 = 22 (все menu_command): 16 add_food + хвосты 3 skip_meal + 3 edit_waist. **Этап 2: 🍽 мигрирована в Python (mig 497 LIVE, PR #380 green, НЕ merged)** → headless `add_food_prompt` + `cmd_add_food` в process_user_input fast-path + router 4c. После merge+0-exec → DELETE 04_Menu (Этап 3). Ветка "Go to 04.2" удалена Safe PUT 2026-06-09. |
...
- **04_Menu (legacy) — на пути к выключению (Этап 2 сделан, Этап 3 ждёт).** НЕ из-за Edit Meal (в Python). Разбор остатка (2026-06-09): после late-May whitelist-фиксов остаток усох до **одного живого входа — reply 🍽 add_food** (с 01.06 = 22 события/нед: 16 add_food + хвосты 3 skip_meal + 3 edit_waist, последние — артефакты момента деплоя, whitelist'ы уже шлют в Python). **Этап 2 (mig 497 LIVE, PR #380 green НЕ merged):** 🍽 add_food → Python headless `add_food_prompt`. **Этап 3 (после merge+deploy+мониторинг 0 exec):** Safe PUT убрать `01_Dispatcher→Go to 04_Menu` → DELETE 04_Menu (n8n 3→2) → убрать мост TD-#18 (`_maybe_pre_delete_for_add_food`). Брифинг — [[handover/2026-06-09_add-food-python-migration]].
