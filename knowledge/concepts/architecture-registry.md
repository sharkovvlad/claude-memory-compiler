...
**Status:** live-аудит обновлён **2026-06-12** — осталось **2 n8n workflows** (`01_Dispatcher` fallback + `04_Menu_v3` headless dispatcher). **2026-06-12:** удалён `04_Menu` (legacy) — последний вход 🍽 add_food мигрирован в Python (mig 497, #380); DELETE блокировался n8n `workflow_published_version` FK (см. [[n8n-data-flow-patterns]]§published_version). **2026-06-09:** удалены `04.2_Edit_StatsDaily` + `08.3_Friends`. **2026-06-08 (Stage 7c):** удалены `03_AI_Engine` + `06_Indicator_Clear`; endpoint `/internal/food_log/render` (#374). Секции с датами ниже — исторические. **Source of truth для агентов:** какой target обслуживает Python authoritative, какой fallthrough'ит на legacy n8n.
...
| ~~`JQsipPWxijse3F0b`~~ | ~~**04_Menu** (legacy)~~ | — | 🔴 **DELETED 2026-06-12.** Последний живой вход (reply 🍽 add_food) мигрирован в Python (mig 497, headless `add_food_prompt`, #380). Safe PUT убрал `01_Dispatcher→Go to 04_Menu`. DELETE блокировался `workflow_published_version` FK (RESTRICT) → удалена строка из живой БД n8n → API DELETE. Backup: `/home/noms/n8n/data/database.backup-20260612.sqlite`. Снапшот: `n8n_workflows/04_Menu*.json` (06-09). |
...
- **04_Menu (legacy) — УДАЛЁН 2026-06-12 (n8n 3→2).** Полная история: разбор остатка → остался один вход 🍽 add_food → mig 497 headless `add_food_prompt` (#380) → Safe PUT убрал `01_Dispatcher→Go to 04_Menu` → DELETE (через обход `workflow_published_version` FK) → мост TD-#18 удалён (#384). Брифинг — [[handover/2026-06-09_add-food-python-migration]]. **Осталось 2 workflow:** `01_Dispatcher` (legacy fallback, forward_to_n8n=0 — простаивает) + `04_Menu_v3` (headless dispatcher).
