...
## 2026-05-13 — JSON boundary type-erosion (`callback_message_id` bug)

**Symptom (UAT 13.05):** клик «Исправить» под карточкой лога еды → карточка **исчезает** (deleteMessage) + bot шлёт новое сообщение с prompt'ом. Не seamless editMessageText.

**Logs:** `template_engine: replace_existing without callback_message_id → fallback to delete_and_send_new`.

### Root cause — type-erosion на JSON границе

`process_user_input` инжектит `callback_message_id` в telegram_ui через:
```sql
v_telegram_ui := v_telegram_ui || jsonb_build_object('callback_message_id',
    COALESCE(p_cb_context->>'callback_message_id', ''));
```

Оператор `->>` возвращает **TEXT**. Int `4775` от Python → `p_cb_context` JSONB integer → `->>` → string `"4775"` → jsonb_build_object кладёт **string** в telegram_ui.

Python `_coerce_message_id` (mig 214 era):
```python
def _coerce_message_id(value):
    if isinstance(value, int) and value > 0:
        return value
    return None  # ← str "4775" сюда попадал
```

Принимал только `int`. String "4775" → None → template_engine fallback на `delete_and_send_new`.

### Почему другие handlers работали

`handlers/onboarding_v3.py` + `handlers/location.py` обходили это **пост-фактум int-инжектом** перед `render_envelope`:
```python
if decision.callback_message_id is not None and decision.callback_message_id > 0:
    telegram_ui["callback_message_id"] = decision.callback_message_id  # raw int
```

Перетирали string из PUI на raw int. `menu_v3.py` НЕ имел этого паттерна → bug проявлялся только для menu_v3 → process_user_input flow (Profile v5 + headless callbacks).

### Fix (mig 214 PR #64) — universal Python

Расширили `_coerce_message_id` чтобы принимал numeric strings:
```python
def _coerce_message_id(value):
    if isinstance(value, bool):  # bool ⊂ int в Python — явно исключаем
        return None
    if isinstance(value, int) and value > 0:
        return value
...
Universal — покрывает всех current + future callers без копи-паста post-RPC injection в каждый handler. Backward-compatible (раньше str отвергался → теперь принимается как numeric).

### Альтернативы (отвергнуты)

- **SQL fix `->>'…'` → `->'…'`** (numeric preservation): затрагивает PUI + process_onboarding_input + других callers, риск выше + потеряли бы type-safety на API boundary (TEXT защищает от случайных JSON-объектов в этом месте).
- **Локальный — копия пост-фактум инжекта в menu_v3.py**: технический долг — будущие handlers повторят bug.

### Rule for future agents

- **Любой JSON-boundary point между SQL и Python — потенциальный type-erosion.** `jsonb` operators (`->>`, `->`, `#>>`, `#>`) имеют разные return types. `->>` всегда TEXT, `->` сохраняет jsonb type. Если данные **должны** быть int/float — используй `->` либо явный CAST на SQL-side, либо tolerant coercion в Python-side.
- **Контракт обоих сторон.** Если SQL guarantees `->>` text — Python coercion mandatory. Если Python expects strict int — SQL должен `(jsonb->'…')::int` cast.
- **Diagnostic recipe:** при «replace_existing without callback_message_id» сначала grep журнал на raw PUI return для подтверждения что int → str.
