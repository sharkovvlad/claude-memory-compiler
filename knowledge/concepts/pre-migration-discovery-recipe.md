...
| **Postgres single-quote escape trap (mig 324 / 327)** | `to_jsonb('foo\nbar'::text)` сохраняет literal 2-char `\, n` — PG standard strings **НЕ интерпретируют** escape sequences. На UX: юзер видит `🍽 13:27 — Котлета\n120 ккал` вместо переносов строк. Затронуло 13 rows × 4 keys (мои migs 324 + legacy onboarding). | **Для строк с `\n`/`\t`/etc — обязательно `E'...'` префикс**: `to_jsonb(E'foo\nbar'::text)`. Или actual newlines в исходнике через chr(10). При написании mig pre-check: `grep -n "'.*\\\\[nt].*'" migrations/<NNN>_*.sql` — должно быть 0 hits без `E` префикса. Recovery via `replace(content::text, chr(92)\|\|chr(92)\|\|'n', chr(92)\|\|'n')::jsonb`. См. [daily/2026-05-24.md] mig 327. |
| **LIKE escape vs replace pattern mismatch (mig 327)** | `WHERE content::text LIKE '%\\n%'` — Postgres LIKE использует `\` как **escape char by default**, поэтому `\\n` в LIKE → literal `\, n` (2 chars), не 3 chars. Над-матчит ВСЕ valid newline escapes JSON source. Verification dies с false positives. | **Для verification of broken `\n` лучше использовать `position(chr(92)\|\|'n' IN leaf_text) > 0` на JSON-parsed leaves**, не LIKE на serialized text. Или ESCAPE clause: `LIKE pattern ESCAPE '\|'`. |
| **LLM payload normaliser must validate against LIVE RPC shape (mig 322)** | `_normalise_day_summary` assumed flat `total_kcal` keys, but real `get_day_summary` returns nested `stats.{total_calories, total_protein, total_fat, total_carbs}` plus `streak_current` (NOT `streak`) plus derived `last_meal_at` from `meals[-1].meal_time`. Silent 0-fallback на 5 из 12 полей → LLM получил «Ноль калорий, впереди целый день» при 1637 ккал/7 meals → owner-visible галлюцинация. | **При wiring RPC → LLM payload: первый шаг — `psycopg2.execute('SELECT <rpc>(...)' )` + `json.dumps(indent=2)` живого ответа.** Mapper пишется ПОСЛЕ того как видел реальную shape, не по spec/assumption. Add `logger.info('SAGE_*_PROMPT tid=%s payload=%r', ...)` from day one для grep'абельных live-debug breadcrumbs. См. [daily/2026-05-24.md] PR #166. |
| **Timezone must be read in EVERY display-time RPC (mig 329)** | `get_day_summary` had `TO_CHAR(MIN(consumed_at), 'HH24:MI')` без `timezone(v_user_tz, ...)` — owner в Madrid (UTC+2) видел 07:43 вместо 09:43. Plus `v_day_start := date_trunc('day', NOW() AT TIME ZONE 'UTC')` — в 02:00 Madrid юзер видел вчерашний день. | **Любой RPC которого output show'ится юзеру через template — ОБЯЗАТЕЛЬНО**: 1) `SELECT COALESCE(timezone, 'UTC') INTO v_user_tz FROM users WHERE telegram_id = p_telegram_id` first; 2) `TO_CHAR(timezone(v_user_tz, ts), ...)` для display; 3) day boundaries: `date_trunc('day', NOW() AT TIME ZONE v_user_tz) AT TIME ZONE v_user_tz`. Audit: `SELECT proname FROM pg_proc WHERE prokind='f' AND pg_get_functiondef(oid) LIKE '%TO_CHAR%consumed_at%'` → каждая функция должна иметь `AT TIME ZONE` рядом. См. [daily/2026-05-24.md] PR #171. |
| **Deploy ordering for breaking schema changes (mig 324/326)** | Apply'нул mig 324 (включая `UPDATE button SET callback_data='cmd_show_meals'`) на live DB до того как Python interceptor для `cmd_show_meals` был deployed. Юзер 30 минут видел мёртвую кнопку. | **Migrations с breaking change в state machine — split**: (a) additive (CREATE TABLE/RPC/screen) apply anytime; (b) breaking (UPDATE existing button cb_data, RENAME RPC, etc.) — apply ТОЛЬКО ПОСЛЕ deploy кода который их поддерживает. См. отдельный [[concepts/migration-deploy-ordering]]. |
