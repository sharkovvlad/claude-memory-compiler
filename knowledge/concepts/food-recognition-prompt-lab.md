...
5. **IN vegetarian-default НЕ применяется.** ~30% Индии non-veg (mutton/chicken-рынок огромный), default «paneer вместо mystery meat» = data-quality regression. Если будет нужен diet-aware prior — это **через `ctx.diet_type`** (omnivore/vegetarian/vegan), не country. Реализовано как пункт 6 (см. ниже).
6. **Diet-aware soft prior через `ctx.diet_type`** (LIVE 2026-06-13, second commit того же дня). Schema verified live: `users.diet_type TEXT CHECK IN (omnivore, vegetarian, vegan)`, nullable. Distribution на момент patch: 15 omnivore / 0 vegetarian / 0 vegan (real users); patch dormant до LATAM/EU launch.
   - **vegetarian:** при genuinely ambiguous photos (mystery filling в dumplings/ravioli/sandwiches, sauced bases, unidentifiable ground component) — prefer plant-based interpretations (tofu, lentils, beans, cheese, paneer, tempeh, seitan) over animal products. **Soft tiebreaker, NOT hard override** — clearly visible meat остаётся честно классифицированным (guest's plate scenario).
   - **vegan:** то же + `including dairy and eggs` exclusion clause. Plant examples без cheese/paneer (добавлены plant milks/spreads). Extra honesty cues — visible egg yolk/white, melted cheese strands, cream sauce gloss → identify honestly если присутствуют.
   - **omnivore / None** → diet-block отсутствует (никаких bias'ов, hint остаётся чисто halal-prior + regional-omonym).
   - Pattern: defensive `getattr(ctx, "diet_type", None) or "" → .lower()` — зеркало Sage convention ([services/sage.py:1671](services/sage.py:1671)). 3 unit-тесты (`test_location_hint_vegetarian_prefers_plant_based_at_ambiguity`, `_vegan_excludes_dairy_and_eggs`, `_omnivore_skips_diet_block`) — anchor против будущих edit'ов.
