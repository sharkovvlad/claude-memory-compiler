...
> **✅ ВЫПОЛНЕНО 2026-06-09 — удалены `04.2_Edit_StatsDaily` + `08.3_Friends`. n8n 5→3 workflows.**
> Расследование (журнал бота 30д + n8n SQLite execution_entity + код Python + NLM):
> - **08.3_Friends** (`su5JZbUXOgE614Lo`, inactive): сирота — 0 executeWorkflow-ссылок во всех workflow; внутри НЕТ payout-логики (только RPC Get Referral Info → Build Friends → Edit Friends). Подозрение про `admin_payout_(approve|reject)_*` СНЯТО — payout обрабатывается в Python `handlers/admin_stars.py` (router ADMIN_PAYOUT_RE → target `admin_payout`). DELETE напрямую (snapshot → API DELETE → HTTP 200).
> - **04.2_Edit_StatsDaily** (`wgY05rXde1PbszSk`): все триггеры мигрировали в Python. `cmd_show_meals` (63 нажатия/30д) → menu_v3 `target=menu_v3 reason=profile_v5_callback` (headless `meals_picker`/`get_meals_picker_data`); `cmd_view_meal_`/`cmd_del_meal_`/`cmd_start_edit_` = **0 за 30д** (Edit Meal cutover 06-08). NLM подтвердил headless: meal detail=`meal_action`/`get_meal_action_data`, daily stats=`stats_main`/`get_daily_stats_rpc`. Достигался только через `04_Menu → "Go to 04.2"`.
> - **Safe PUT `04_Menu`** (335 КБ, через `urllib`+PUT, GET-перед-PUT): удалены 2 ноды `Go to 04.2` + `Prepare for 04.2` (124→122). Проводка была изолирована: `Menu Router [out#3 "meals"] → Prepare for 04.2 → Go to 04.2 (тупик)`. **Технику запомнить:** ветку Switch #3 НЕ удалять (сдвинет позиции 26 остальных выходов), а **опустошить → `[]`** — мёртвое правило «meals» в Switch безвредно (как dead-cases в 04_Menu_v3). Все 27 выходов сохранены, active=True. Затем DELETE 04.2.
> - **PR #378** — снапшоты в `n8n_workflows/` (04.2, 08.3, 04_Menu_pre_042_removal). Деплоя НЕ требует (правки в живом n8n).
> - **04_Menu ОСТАВЛЕН** — живой fallback (96 событий/2нед, реальные юзеры): `cmd_wellbeing_today`, `cmd_diet_*`, `cmd_cycle_view`, `cmd_edit_waist`, `cmd_skip_meal`, главные reply-кнопки. **Durable урок (уточнение гейта):** для executeWorkflow-only sub-workflow'а (без живого webhook-входа) сильнейший гейт необратимого DELETE — **структурная недостижимость**: убрать ВСЕ executeWorkflow-ссылки Safe PUT'ом → путей вызова физически не остаётся (надёжнее «подождать N дней», т.к. устраняет сам механизм, который executeWorkflow-игнор-active эксплуатирует). Обязательна полная ref-проверка по ВСЕМ workflow перед удалением.
