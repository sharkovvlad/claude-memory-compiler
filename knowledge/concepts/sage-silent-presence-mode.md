...
## Card S ≠ META — это две половины одной фичи (частый вопрос)

Owner-вопрос 2026-06-13: «Card S и `_silent_presence_meta` — это одно и то же?» Ответ: **одна фича, две части**.

| Часть | Роль | Где живёт | Когда «активна» |
|---|---|---|---|
| **`_silent_presence_meta()`** (Python) | **Выключатель + условие.** Решает КОГДА входить в режим тишины и инжектит инструкцию в payload | `services/sage.py`, вызывается в `_build_user_prompt` | Только когда master-flag `true` И 4 условия сошлись |
| **Card S** (текст) | **Образец голоса.** Показывает модели КАК звучать в режиме тишины | В `_DEFAULT_SYSTEM_PROMPT_EN` среди VOICE EXAMPLES (Cards A-H, P, S) | **Всегда** в промпте; «срабатывает» только когда META на неё сошлётся |

**Аналогия:** META = команда «замолчи сейчас» (условная). Card S = «вот так звучит молчание» (всегда под рукой). Карточка сама по себе ничего не меняет — это просто ещё один пример голоса. Без META модель не знает, что пора входить в режим. **Это generic-паттерн всего META-каскада** ([[sage-payload-meta-override-pattern]]): voice card = reference example (всегда в промпте), META = conditional trigger (инжектится в payload). Card D + `quiet_steady_no_push` — та же пара на my_day surface.

## Почему master flag default `false` — рацио (durable)

Не из осторожности — из **чистоты эксперимента**.

В PR-3b (#401) на прод уехало НЕСКОЛЬКО изменений, которые работают **безусловно** (без флага): Goal в payload, Last log min, Card P, FORBIDDEN refinement. Они уже меняют тон Номса. Если поверх них сразу включить `silent_presence` — на watch-периоде **невозможно атрибутировать**, что на что повлияло (доля smirk, клише, появление proud). Перемешаются причины.

Грамотный rollout: **дать безусловным изменениям отстояться (24-48ч watch)** → снять метрики → **потом отдельно щёлкнуть `silent_presence`** как изолированный single-variable опыт. Флаг глобальный (не per-user), активных живых юзеров ~4, откат — одна строка hot-reload (мгновенно), так что риск самой активации минимальный — но изоляция переменной важнее скорости. **Durable lesson:** когда несколько tone-изменений едут в одном PR, новые поведенческие фичи class'а «изменение behavior» гейти за flag и включай по очереди, чтобы watch-период читался.
