...
4. **Canonical entitlement check:** `user_has_active_premium(tid)` RPC (mig 424). НЕ `users.subscription_status` (denormalized cache, drift-prone) и НЕ `user_has_renewable_sub` (см. ниже — это «продлится ли», не «есть ли доступ»).
