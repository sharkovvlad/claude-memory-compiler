...
> ⚠️ **status: частично superseded (2026-06-05).** Архитектура (3 оси: training_type/phenotype/
> goal_speed; RFM→LBM→Katch; всё в app_constants) — актуальна. НО **сам макро-сплит переписан**:
> жир теперь = ТАРГЕТ г/кг + коридор (не `fat_pct_*` %), белок deficit-aware + верхние потолки/
> vegan-cap, углеводы = коридор, reference-вес для ожирения (v15, mig 462); PAL14-энергия (v16,
> mig 464). **Актуальная формула и значения → [[calc-scientific-basis]] §«Консенсус v15+v16».**
...
### v12 — mig 427/428 (2026-06-02): RFM/Katch для ВСЕХ фенотипов + LBM-матрица белка + waist UI

Applied LIVE 2026-06-02 (PR #289). Owner+PM approved по документу «Глубокий анализ формулы расчёта целей NOMS» (Шаг 3/4/6). Факты верифицированы в источниках (RFM Woolcott-Bergman 2018 NHANES/DEXA; Katch 370+21.6×LBM; белок Helms 2.3-3.1 г/кг FFM при дефиците).

- **RFM→LBM→Katch теперь для ЛЮБОГО фенотипа** при наличии `waist_circumference` (было: только `obese`-ветка mig 295). Без талии — поведение не меняется (нулевая регрессия; в проде 0 юзеров с талией).
- **LBM-матрица белка** (`app_constants protein_g_per_kg_lbm_*`: sedentary 1.6 / none 1.6 / cardio 2.0 / mixed 2.2 / strength 2.4 г/кг **LBM**) — применяется ТОЛЬКО на RFM-ветке (`v_lbm_kg IS NOT NULL`); иначе legacy total-weight матрица. **Coupled с RFM:** без повышенных коэф. переход на меньшую базу (LBM) занизил бы белок (owner с талией 99: 1.4×LBM=104г было бы слишком мало → 2.0×LBM=148г правильно).
- **Жир-floor → ФАКТИЧЕСКИЙ вес** (`v_user.weight_kg`), был на `v_target_weight` (DR Шаг 6: эндокринная буферизация зависит от ОБЩЕЙ массы, не LBM). Затронул 1 monw-юзера (жир 48→56г) — коррекция, не регрессия.
- **UI (mig 428):** кнопка «Талия» в `[📏 Мои Данные]` (`personal_metrics`, для всех, re-editable) + онбординг waist открыт всем фенотипам + `buttons.edit_waist` ×13.
- **'mixed'/'none' LBM-коэф. (2.2/1.6) — интерполяция агента** (PM дал sedentary/cardio/strength); hot-reload, легко скорректировать.

**Fix C — РЕШЁН этой миграцией** (в LBM-контексте). Для non-RFM пути (без талии) 1.4 г/кг для кардио остаётся защитимым; для RFM-пути — LBM-матрица. Goal-aware floor для total-weight пути НЕ внедрён (не понадобился — RFM покрывает кейс точнее). История гипотезы ниже сохранена для контекста.

---

## ✅ RESOLVED (был OPEN QUESTION) — goal-aware protein (2026-06-02)

> Решено через mig 427 (LBM-матрица на RFM-ветке), см. §v12 выше. История ниже — как пришли к решению (изначально гипотеза была подана слишком категорично, потом переформулирована, потом закрыта документом owner'а).

**Наблюдение.** Белок задаётся только по `training_type` (`protein_g_per_kg_cardio=1.4`). У owner'а (М, 99 кг, кардио, цель lose): 1.4×99 = 139 г (21% kcal). Углеводы = остаток → 344 г (52% kcal). Owner заметил «углеводы высокие».

**Почему это НЕ очевидный баг (честная наука):**
- Для **выносливости/кардио** доказательная норма белка = **1.2–1.6 г/кг**. Не 1.6–2.2 — это для **силовых** на сушке (Helms 2014, ISSN Jäger 2017). **1.4 г/кг для кардио — внутри нормы, защитимо.**
- Аргумент за повышение — не тип тренировки, а **дефицит** (сытость + удержание LBM). Но для кардио-онли честный потолок поправки ~**1.6 г/кг**, НЕ 1.8–2.2 (это была переоценка в первой формулировке).
- **На какой вес считать г/кг** — отдельный нерешённый вопрос: у юзера с лишним весом г/кг от total bodyweight завышает; норма — LBM/целевой вес. Уже зафиксировано как **P1.3 Adjusted Body Weight** ([[calc-user-targets-roadmap]], отложено). Калькулятор считает белок на `v_target_weight`.

**Про «углеводы завышены».** 52% — не отдельный баг, а следствие remainder-расчёта (`v_carbs = (kcal − protein_kcal − fat_kcal)/4`) при умеренном белке. Проценты — **по калориям** (AMDR-стандарт), НЕ по массе (по массе углеводы = 61%; типичная путаница). Жир 79 г = floor `fat_min_g_per_kg=0.8` (сработал `floor_triggered_fat`) — научно обоснован, НЕ трогать.

**Что нужно ДО любого изменения (gate):**
1. Сверка с первоисточником `Разработка алгоритмов питания…md §макро` — что он реально предписывает по белку при дефиците.
2. Валидация в Digital Twin — но он **отстал (v8 vs live v11)**, сначала досинк ([[calc-user-targets-test-spreadsheet]] §версии). Owner: править твин позже, когда останется только Adaptive TDEE.
3. Клиническое слово owner'а (он нутрициолог).
4. Регрессия: не пробьёт ли рост белка `min_kcal_floor`/`carbs_min_g`; РПП-safety (underweight/cachexia guards).
5. Отдельный PR/миграция, НЕ смешивать с mig 424.

**Возможное решение (ЕСЛИ (1)-(3) подтвердят):** goal-aware floor — `v_protein := GREATEST(ROUND(training_baseline*v_tw), ROUND(deficit_floor*v_tw))` при `goal_type='lose'`, `deficit_floor` ~1.6 г/кг (не 1.8) через новый `app_constants` (hot-reload). Но это гипотеза, не план.

## Научный аудит v13 (2026-06-03) — Digital Twin + рекомендации (НЕ применено)

> 📌 **Консенсус v14 достигнут** (мой аудит + внешний ИИ + владелец, 2026-06-03) — каноничное
> обоснование и итоговое направление вынесены в [[concepts/calc-scientific-basis]] (recent правит).
> Ниже — durable выводы аудита; при расхождении приоритет у `calc-scientific-basis`.

Владелец-driven аудит «соответствует ли расчёт науке». Построен **Digital Twin v13** (21 профиль,
провалидированы 1:1 против живого RPC, дельты 0). Деливераблы — `Нутрициолог (аудит расчётов v13)/`
(untracked корень NOMS) + нативный Google Sheet (Drive id `1GWSD4YiNkbkN_UQpOTlTNsEnNdKogHOS9STT9BG6zgw`).
Полный аудит с цитатами — `AUDIT_v13_findings_and_recommendations.md`. Внешний ИИ ещё не запускался.

**Durable выводы (с первоисточниками):**

1. **PAL cap 1.8 — мёртвый код.** Реально достижимый максимум = `pal_heavy 1.725` (training_bonus
   heavy = 0; sed 1.2+.15=1.35, light 1.375+.10=1.475, mod 1.55+.05=1.60, heavy 1.725+0=1.725).
   `LEAST(...,1.8)` никогда не срабатывает. **Нет Extreme-категории** → `heavy+cardio+lose`
   недокормлен: профиль 70кг/марафонец → target 2408 при истинном TDEE ~3245 (MET), **EA≈25 < порога
   RED-S 30** (Loucks 2003 JCEM 88/1/297). Safety-adjacent. Fix: `pal_extreme` ~1.9–2.2 + EA-guard
   (нужен сбор минут тренировок). FAO/WHO 2004: vigorous PAL 2.0–2.4; элита до 4.0.

2. **«Высокие углеводы» (жалоба владельца) — следствие ЗАНИЖЕННОГО БЕЛКА, не «мало жира».**
   Углеводы = остаток; белок умеренный + жир на floor → углеводы 50–65% ккал. НО грамм-floor жира
   0.8 г/кг **структурно ВЕРЕН**: на дефиците потребность в жире не масштабируется с калориями →
   абсолютный грамм-floor лучше %-таргета (StrongerByScience; Whittaker 2021 meta S0960076021000716:
   low-fat ↓ тестостерон SMD −0.38). Корень — белок НЕ deficit-aware: кардио 1.4 г/кг массы /
   2.0 г/кг LBM ниже Helms 2014 (PMID 24092765) 2.3–3.1 г/кг **FFM** и Longland 2016 AJCN (2.4 vs
   1.2 г/кг → +1.2 vs +0.1 кг мышц). **P1-fix:** deficit-aware белок (`GREATEST(training_baseline,
   ~1.8 г/кг массы)` на `lose`) → углеводы-остаток схлопываются сами. На owner #05: 23/27/50 → 27/31/42.

3. **База PAL sedentary 1.2 занижает.** NASEM 2023 DRI (NBK591020): «inactive» 1.2–1.5, где **1.2 =
   прикованный к постели**, реально sedentary free-living ≈ 1.4. Fix: `pal_sedentary` 1.2→~1.35–1.4
   (осторожно — было «защитным» занижением для truly inactive).

4. **НЕ трогать (соответствует науке):** беременность/лактация, Molnar/Schofield/Lührmann, min-kcal
   floor, BMI/age/РПП-guards, RFM (Woolcott-Bergman), Katch, **грамм-floor жира 0.8 г/кг**. ISSN
   (PMC5470183): «единого правильного макро-соотношения нет» → углеводы-остаток для композиции не баг.

5. **Diet breaks (P2):** MATADOR Byrne 2018 (PMID 28925405): 2-нед перерывы → 14.1 vs 9.1 кг,
   адаптация −360 vs −749 кДж/д. Caveat: Campbell re-analysis (PMC7739336) — нет значимой разницы по
   total FFM. **Adaptive TDEE (P3):** стратегический сдвиг (MacroFactor EWMA), большая работа.

**Lesson — Digital Twin version drift:** старая twin была **v8.3**, прод — **v13** (дрейф 5 версий).
Всегда сверять версию twin против live `pg_get_functiondef` ПЕРЕД использованием. Новый v13-twin
провалидирован headless (`formulas` lib) + против RPC. Recipe — [[concepts/calc-user-targets-test-spreadsheet]].
