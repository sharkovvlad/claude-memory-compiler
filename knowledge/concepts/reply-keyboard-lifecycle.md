...
## Новая кнопка в main reply-kb → как доходит до СУЩЕСТВУЮЩИХ юзеров (2026-06-03)

Добавил кнопку в `build_main_reply_keyboard()` (mig 453, barcode `[🔎 Штрихкод]`) — RPC
и Python собирают kb **свежей**, но существующие активные юзеры видят **старую
закэшированную** клавиатуру. Жёсткий факт Telegram: **обновить reply-kb существующему
юзеру можно ТОЛЬКО отправив сообщение с новой kb** — «тихого» push нет (chat-level кэш).

**Кто получит новую кнопку и когда:**
- **Новые юзеры** — сразу (онбординг `onboarding_success_menu` unconditional attach).
- **Premium активные** — в течение суток: ежедневный sleep/stress чек-ин → ответ юзера →
  `pending_kb_reattach` (mig 374) → reattach едет на ответе (не лишний пузырь).
- **🔴 FREE активные — авто-пути НЕТ.** `sleep_checkin`/`stress_checkin` **премиум-гейтнуты**
  (`cron_get_reminder_candidates`, mig 330: `user_has_active_access(subscription_status)`)
  → mig 374 recovery до free не доходит. Кроны напоминаний — inline, не reply-kb (GAP выше).
  Free получит кнопку только когда САМ сменит язык (lang-refresh) / отредактирует профиль-число
  (conditional reattach). Скан еды/штрихкода kb НЕ возвращает (food-card = WONTFIX).

**Вывод для агента, добавляющего кнопку в main reply-kb:** не жди что она «появится у всех
сама». Premium — через чек-ин, free — только по их собственному действию. Форсить у всех =
один сервисный пузырь каждому (`pending_kb_reattach` broadcast) — обычно owner против (спам).
Если кнопка = discoverability к фиче, которая работает и без неё (как barcode auto-detect) —
**ничего не форсить**, разойдётся естественно. KB [[barcode-logging-openfoodfacts]] §8.
