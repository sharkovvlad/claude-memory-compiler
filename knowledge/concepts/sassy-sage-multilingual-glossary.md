...
> ⛔ **Нет отдельной сущности «Pet»/«Пет».** Компаньон = сам бот NOMS. Слово «Pet»/«Пет» в текстах ЗАПРЕЩЕНО (owner rule 2026-06-07): пользователь не понимает, кто такой Pet и где его искать — это фантомная сущность (уже вычищалась в mig 278, но осталась в `sage.guarded.*`). Можно будет ссылаться на конкретного питомца ТОЛЬКО когда появится приложение с визуальным Pet на экране, и ТОЛЬКО на тех экранах, где он реально присутствует. До тех пор — никаких «Pet approves / Пет улыбается». Подробно — [[concepts/phantom-pet-entity]].
...
## Addendum — payout/shop i18n (2026-06-07, mig 487)

Транзакционные money-экраны (вывод средств + магазин) локализованы ×11. Два durable-трапа, выявленных critic-субагентами:

**AR — gender-neutral ≠ выкинуть все `ـك`.** Possessive-clitic `ـك` (`رقمك`, `رصيدك`, `محفظتك`) и `يمكنك` **без огласовок визуально идентичны** муж/жен → уже нейтральны, не трогать. Гендерятся только **императивы** (`انتظر`→`انتظري`, `تأكد`→`تأكدي`, `حاول`/`أرسل`/`اضغط`/`ادعُ`/`اكسب`/`احصل`). Нейтрализуй ТОЛЬКО их: `يُرجى`+masdar (`يُرجى الانتظار`, `يُرجى إرسال`) или `يمكنك`+verb (`يمكنك دعوة`). Для money-экранов plain MSA уместнее, чем relaxed pan-Arab сленг.

**FA — clitic `ـت` на قاموس-словах → омоним-коллизии.** `موجودی`(balance)+`ت`=`موجودیت` → реальное слово «entity/existence»; `مانا`+`ت`=`مانات` → валюта manat (Азербайджан/Туркмения). Юзать **раздельную** форму `موجودی تو` / `مانای تو`. Перед любой FA clitic-формой проверяй на омоним.

**Структура `ui_translations.content` — вложенная:** `payout.confirm_text` = `content->'payout'->'confirm_text'`, jsonb_set path `'{payout,confirm_text}'`. НЕ flat top-level ключ.

## Addendum — STREAK naming Python-side guard (PR #402, 2026-06-13 EOD)

**Owner observed:** 4/5 RU my_day реакций gpt-4o-mini за 24h писали «Стreak»/«стрика»/«стрике» (mixed-script / запрещённая транслитерация) вместо canonical «серия». Источник — payload-line `Streak: N days` сидит **рядом** с числом, модель копирует/транслитерирует токен под heavy non-EN context. Запрет в `_DEFAULT_SYSTEM_PROMPT_*` FORBIDDEN clause (line 2322, 2437 `services/sage.py`) существует, но погребён глубоко в промпте — низкая salience для end-of-context-weighted gpt-4o-mini.

### Fix (двухчастный)

1. **Payload-line rename:** `«Streak: 42 days»` → `«Day streak count: 42»` в `_build_user_prompt` (food_log) и `_build_my_day_prompt` (my_day). Убирает буквальный токен из data-section.
2. **`_streak_naming_meta(language, streak_n)`** в `services/sage.py` — HARD GUARD приклеивается AFTER `_language_lock_meta` (самый tail, max salience). Содержит native-слово на языке юзера + явный банлист («стрик», «стрика», «стрике», «Стreak», «стрік», «ستريك», «استریک», «Streak») + warning про mixed-script.

### Native-word mapping (anchor — Sage prompt FORBIDDEN clause line 2322/2437)

| Lang | Native | Источник |
|---|---|---|
| ru | серия | Sage prompt + KB этой страницы |
| uk | серія | Sage prompt + KB |
| ar | سلسلة (silsila) | Sage prompt + KB §AR line 1850 |
| es | racha | Sage prompt + KB §ES line 777 |
| fr | série | Sage prompt + KB §FR line 273 |
| it | serie | Sage prompt + KB §IT line 439 (nei формальном) |
| de | Serie | Sage prompt (override KB §DE «Streak» — UI-chip оставляет «Streak» по KB, but Sage runtime реакции форсируют «Serie» через META) |
| pt | sequência | Sage prompt + KB §PT line 603 |
| pl | seria | Sage prompt + KB §PL line 1107 (nei формальном; KB-«passa» — primary, но Sage prompt пинит «seria») |

**FA/HI/ID — intentionally НЕ в META mapping.** Sage prompt не пинит native для них; KB-glossary даёт mixed signals (FA-«استریک» KB допускает gym-slang, но Sage prompt банит; HI/ID — KB разрешает англицизм). Существующая FORBIDDEN clause работает как fallback для этих langs.

### UI-хардкод audit (parallel)

`tools/audit_streak_literal.py` — reusable read-only audit `ui_translations` на streak-токены + cyrillic-транслитерации (case-insensitive). DE/EN исключены (KB-санкционированный англицизм).

**Mig 507** заменил 4 хардкода в `progress.skip_streak_chip`:
- UK: «🏆 Стрік: {streak}» → «🏆 Серія: {streak}»
- FA: «🏆 استریک: {streak}» → «🏆 سلسله: {streak}» (silsileh — Persian cognate AR-silsila)
- HI: «🏆 Streak: {streak}» → «🏆 सिलसिला: {streak}» (silsila — KB §HI line 1670)
- ID: «🏆 Streak: {streak}» → «🏆 Seri: {streak}» (Indonesian для «series»; «rentetan» из KB §ID академично)

### Verification

| Метрика | Pre-fix | Post-fix |
|---|---|---|
| Owner RU реакций с правильной формой (24h, telegram_id=417002669) | 1/5 | dry-run 5/5 ad-hoc + dry-run-protocol 16/16 |
| `tools/audit_streak_literal.py` UI hits | 4 (UK/FA/HI/ID) | 0 clean |
| Sage tests | 200 | 221 (+20 dedicated `test_sage_streak_naming.py`) |

### Durable lesson

**Промпт-side FORBIDDEN clause недостаточен** для gpt-4o-mini когда конкретный token-источник сидит в payload рядом с ключевым числом. Reliable fix = (a) убрать токен из payload, (b) добавить per-language tail-anchored HARD GUARD с конкретным native и явным банлистом. Apply pattern для любого аналогичного класса bug'ов («модель копирует payload-token буквально в non-EN output»). Pattern сompose'ится со всеми остальными META через cascade ordering.

### Источники в коде

- `services/sage.py:_streak_naming_meta` + `_STREAK_NATIVE_WORD`
- `migrations/508_streak_native_words_audit.sql` (originally apply'нут как 507, renumbered после CI sanity-guard collision §12.1)
- `tools/audit_streak_literal.py`, `tools/sage_dry_run_streak_only.py`
- [PR #402](https://github.com/sharkovvlad/noms-bot/pull/402)
