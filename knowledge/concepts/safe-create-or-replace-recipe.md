...
| **403** (31.05) | `get_subscription_business_data` | Добавляли поле `expires_at_iso`. Базу взяли **из `grep expires_at` фрагмента** оригинала, НЕ через `pg_get_functiondef` целиком. Не увидели что mig 362/364 определила RPC с **14 pre-resolved полями** для headless экрана «Моя подписка» (`plan_label`, `payment_method_line`, `access_until_line`, `auto_renewal_line`, `days_remaining_line`, `sage_line_contextual` + Stars-recurring поля). `CREATE OR REPLACE` укоротил RPC до 5 полей — Python начал получать `None` и шаблон рендерил литералы `{plan_label}` и пр. Также ошибочно добавили `AND cancelled_at IS NULL`, что исключало cancelled-but-still-active subscriptions (admin Stripe `cancel_at_period_end`). | Production регрессия: экран «Моя подписка» показывал юзеру плейсхолдеры вместо плана/даты/способа оплаты. Hotfix **mig 408** через ~2 часа после merge PR #256 — full restore body из mig 364 + добавление 2 новых полей **без удаления существующих**. 4-й случай в таблице: **grep-фрагмент для CREATE OR REPLACE = всегда регрессия**, даже когда «вроде ничего не трогаю». |

**Общий root cause всех четырёх случаев:** агент не проверил, что его snapshot — **актуальное** живое состояние прода **в момент применения**, а не «было правдой пять минут назад» (или того хуже — был только фрагментом тела, как в 403).
