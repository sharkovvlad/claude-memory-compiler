...
| 2 | PR-3c | Sage видит **тренд**, не один день: weekly macro pattern + days_at_target | **pending** |
| 3 | PR-3d | Sage уважает юзера: «цель vs реальный рацион» divergence hint | **pending** |
...
## Phase 2 (PR-3c) — детальный план

**Цель:** Sage видит паттерн за 7 дней, а не один день. Самое нутрициологически ценное звено.

Два сигнала, **один RPC sweep** (атомарность):
- **Weekly macro pattern** — «5/7 дней protein on pace» / «3 дня подряд carbs overload». Главный нутрициологический сигнал — позволяет работать с **трендом**, а не разовым промахом (что и зашито в tone-guide Sage).
- **days_at_target** — «3+ дня подряд в цели» → явный триггер для emotion=proud (сейчас модель выбирает proud на глаз; Card P уже есть, нужен сигнал-якорь).

**Реализация:** расширить `get_day_summary` RPC ИЛИ новый дешёвый RPC (7-day агрегат по `daily_*` / day-status истории). **NLM-first** — проверить какие 7-day агрегаты УЖЕ есть, точные имена таблиц (`day_summary` сейчас только за день). Если новый RPC — **p95 benchmark обязателен** (≤50ms или эскалировать).

**Payload:** компактная строка `Recent pattern: ...` ТОЛЬКО когда паттерн есть (не дамп всех макро каждый день). Новая Card (pattern call-out): «Four days in a row carbs walking the show. Pattern, not slip.»

**Dry-run × 2** обязателен (новая card + payload-строка). Snapshot + unit на helper.
