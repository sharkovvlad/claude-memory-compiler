...
**Status:** ⚠️ ПЕРЕСМОТРЕНО (owner, тот же день, позже). Owner уточнил: распознавание еды для НЕзавершивших регистрацию уже работает и это НАМЕРЕННО (вовлечение/удержание, best practice — «дать попробовать с первой секунды»). **Жёсткий trial-лимит/гейт строить НЕ нужно** — он противоречит этой философии. На столе остаётся максимум МЯГКИЙ неблокирующий nudge («круто! доделай настройку, чтобы открыть стрики/статистику») после N логов — и то под вопросом, не приоритет. 4 ответа ниже относились к ранее-планировавшемуся гейту, который теперь отложен/отменён. Не реализовывать без явного нового запроса owner'а.
